...
$(function() {
	...
	$( "#wineSlider" ).slider({
		create: function() {
      var percent = 100 - $(this).slider("value");
			handle.text( percent + '%' );
		},
		slide: function( event, ui ) {
      var percent = 100 - ui.value
			handle.text( percent + '%' );
			wineData.level =  percent;
			$('#wineResult').val(alcoholCalc(mlWine, percentWine, wineData.level));
	...
	$( "#vodkaSlider" ).slider({
		create: function() {
      var percent = 100 - $(this).slider("value");
			vodkahandle.text( percent + '%' );
		},
		slide: function( event, ui ) {
      var percent = 100 - ui.value
			vodkahandle.text( percent + '%' );
			vodkaData.level = percent;
			$('#vodkaResult').val(alcoholCalc(mlVodka, percentVodka, vodkaData.level));
	...
	$( "#beerSlider" ).slider({
		create: function() {
      var percent = 100 - $(this).slider("value");
			beerhandle.text( percent + '%' );
		},
		slide: function( event, ui ) {
      var percent = 100 - ui.value
			beerhandle.text( percent + '%' );
			beerData.level = percent;
			$('#beerResult').val(alcoholCalc(mlBeer, percentBeer, beerData.level));
...
var wineAlc = 0.769275;
var vodkaAlc = 750;
var beerAlc = 750;
var range = 750
var percentWine = 0.13
var percentVodka = 0.4
var percentBeer = 0.05
var mlWine = 750
var mlVodka = 500
var mlBeer = 500

function alcoholCalc(totalMl, percentAlcohol, alcoholConsumed) {
  var grams = totalMl * percentAlcohol * 0.789 * (1 - alcoholConsumed / 100)
